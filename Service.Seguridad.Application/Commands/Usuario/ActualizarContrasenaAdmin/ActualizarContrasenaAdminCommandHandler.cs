...
public class ActualizarContrasenaAdminCommandHandler : IRequestHandler<ActualizarContrasenaAdminCommand, Unit>
{
    private readonly IUsuarioRepository _usuarioRepository;

    public ActualizarContrasenaAdminCommandHandler(IUsuarioRepository usuarioRepository)
        => _usuarioRepository = usuarioRepository;

    public async Task<Unit> Handle(ActualizarContrasenaAdminCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!ValidarContrasenaFuerte(request.NuevaContrasena))
            throw new ArgumentException("La contraseÃ±a debe tener al menos 8 caracteres, una mayÃºscula, una minÃºscula, un nÃºmero y un carÃ¡cter especial.");
